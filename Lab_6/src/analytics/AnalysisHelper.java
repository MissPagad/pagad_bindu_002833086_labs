...
public class AnalysisHelper {
    ...
    public void getPassiveCommentUsers() {
        ...
        System.out.println("Q5 - The following users have the least posts:  ");
    ...
    public void getPassiveAndActiveOverallUsers() {
        DataStore data = DataStore.getInstance();
        HashMap<Integer, Integer> overllNumbers = new HashMap<Integer, Integer>();

        for (Comment c : data.getComments().values()) {
            int userId = c.getUserId();
            if (overllNumbers.containsKey(userId)) {
                overllNumbers.put(userId, overllNumbers.get(userId) + 1 + c.getLikes());
            } else {
                overllNumbers.put(userId, 1 + c.getLikes());
            }
        }
        for (Post p : data.getPosts().values()) {
            int userId = p.getUserId();
            ...
            if (overllNumbers.containsKey(userId)) {
                overllNumbers.put(userId, overllNumbers.get(userId) + 1);
            } else {
                overllNumbers.put(userId, 1);
            }
        }

        ArrayList<User> users = new ArrayList(data.getUsers().values());
        Collections.sort(users, new UserMapComparator(overllNumbers));

        System.out.println("Q6 - The following users have overall been passive:  ");

        for (int i = 0; i < 5; i++) {
            System.out.println(users.get(i) + ", - Overall count: " + overllNumbers.get(users.get(i).getId()));
        }
